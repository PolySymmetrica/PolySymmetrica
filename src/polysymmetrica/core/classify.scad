...
// Function: _ps_round()
// Usage:
//   result = _ps_round(v, eps);
// Description:
//   Quantize numeric value to epsilon grid.
//   .
//   - Returns: rounded scalar.
//   .
//   - Purpose: stabilize geometric keys against floating point noise.
// Arguments:
//   v = scalar value.
...
// Function: _ps_edge_len()
// Usage:
//   result = _ps_edge_len(verts, e);
// Description:
...
// Function: _ps_unique_keys()
// Usage:
//   result = _ps_unique_keys(keys);
// Description:
//   Keep first occurrence of each key (stable order).
//   .
//   - Returns: list of unique keys.
//   .
//   - Approach: linear first-seen filter preserving input order.
// Arguments:
//   keys = list of comparable values.
function _ps_unique_keys(keys) =
    [
        for (i = [0:1:len(keys)-1])
            if (len([for (j = [0:1:i-1]) if (keys[j] == keys[i]) 1]) == 0)
                keys[i]
    ];

// Function: _ps_key_str()
// Usage:
//   result = _ps_key_str(k);
// Description:
...
// Function: _ps_group_by_key()
// Usage:
//   result = _ps_group_by_key(keys);
// Description:
//   Group element indices by identical key.
//   .
//   - Returns: family list `[[key, idxs], ...]`.
//   .
//   - Approach: stringify keys, unique string set, then collect member indices.
// Arguments:
...
// Function: _ps_family_ids()
// Usage:
//   result = _ps_family_ids(n, fams);
// Description:
//   Build direct element->family-id map.
//   .
//   - Returns: ids array of length `n`; `-1` if unmatched.
// Arguments:
//   n = element count.
...
// Function: _ps_lex_gt()
// Usage:
//   result = _ps_lex_gt(a, b);
// Description:
//   Lexicographic strict-greater comparison.
//   .
//   - Returns: true iff `a > b` lexicographically.
// Arguments:
//   `a` =
//   `b`: equal-length sequences. =
...
// Function: _ps_lex_best()
// Usage:
//   result = _ps_lex_best(list, mode, acc, idx);
// Description:
//   Pick lexicographic extremum from a sequence set.
//   .
//   - Returns: best sequence by chosen order.
// Arguments:
//   list = candidate sequences.
//   mode = `"max"` or `"min"`.
//   `acc` =
//   `idx`: recursion internals. =
...
// Function: _ps_cyclic_canonical()
// Usage:
//   result = _ps_cyclic_canonical(seq, mode, allow_reflect);
// Description:
//   Canonical cyclic representation (necklace/bracelet form).
//   .
//   - Returns: canonicalized sequence.
//   .
//   - Approach: enumerate rotations (and optional reflected rotations),
...
// Function: _ps_face_keys_from()
// Usage:
//   result = _ps_face_keys_from(verts, faces, detail, eps);
// Description:
//   Build base face keys.
//   .
//   - Returns: per-face key list.
// Arguments:
//   `verts` =
//   `faces`: mesh. =
...
// Function: _ps_edge_keys_from()
// Usage:
//   result = _ps_edge_keys_from(verts, faces, edges, edge_faces, detail, eps);
// Description:
//   Build base edge keys.
//   .
//   - Returns: per-edge key list.
// Arguments:
//   `verts` =
//   `faces` =
//   `edges` =
//   `edge_faces`: mesh + adjacency. =
...
// Function: _ps_edge_keys_list()
// Usage:
//   result = _ps_edge_keys_list(edges, nv);
// Description:
//   Encode undirected edges as sortable scalar keys.
//   .
//   - Returns: numeric key list.
// Arguments:
...
// Function: _ps_edge_index()
// Usage:
//   result = _ps_edge_index(edge_keys, a, b, nv);
// Description:
...
// Function: _ps_vert_keys_from()
// Usage:
//   result = _ps_vert_keys_from(verts, faces, edges, edge_faces, detail, eps);
// Description:
//   Build base vertex keys from valence and cyclic incident-face sizes.
//   .
//   - Returns: per-vertex key list.
// Arguments:
//   `verts` =
//   `faces` =
//   `edges` =
...
// Function: _ps_refine_face_keys()
// Usage:
//   result = _ps_refine_face_keys(poly, face_keys, face_nbr_keys, vert_ids, edges, edge_faces, edge_keys, nv);
// Description:
//   Refine face keys by neighboring face-family and vertex-family context.
//   .
//   - Returns: refined face key list.
//   .
//   - Approach: convert neighbor keys into temporary family ids,
...
// Function: _ps_refine_edge_keys()
// Usage:
//   result = _ps_refine_edge_keys(poly, edge_keys, face_keys, vert_keys, edges, edge_faces);
// Description:
//   Refine edge keys using current face/vertex family ids.
//   .
//   - Returns: refined edge key list.
// Arguments:
//   poly = descriptor.
//   edge_keys = current edge keys.
//   `face_keys` =
//   `vert_keys`: current face =
//   vertex keys. =
//   `edges` =
...
// Function: _ps_refine_vert_keys()
// Usage:
//   result = _ps_refine_vert_keys(poly, vert_keys, face_keys, edges, edge_faces);
// Description:
//   Refine vertex keys using incident face-family ids.
//   .
//   - Returns: refined vertex key list.
// Arguments:
//   poly = descriptor.
...
// Function: _ps_keys_equal()
// Usage:
//   result = _ps_keys_equal(a, b);
// Description:
//   Exact elementwise key list equality.
//   .
//   - Returns: true when lengths and all elements are equal.
// Arguments:
//   `a` =
//   `b`: key lists. =
...
// Function: _ps_families_equal()
// Usage:
//   result = _ps_families_equal(keys_a, keys_b);
// Description:
//   Compare partition equality (family membership), ignoring key payload growth.
//   .
//   - Returns: true when induced family-id maps are equal.
// Arguments:
//   `keys_a` =
//   `keys_b`: key lists. =
...
// Function: _ps_refine_face_keys_iter()
// Usage:
//   result = _ps_refine_face_keys_iter(poly, keys, vert_ids, edges, edge_faces, edge_keys, nv, max_iter);
// Description:
//   Iterative face-key refinement until stable families or budget exhausted.
//   .
//   - Returns: refined face keys.
// Arguments:
//   poly = descriptor.
...
// Function: _ps_refine_edge_keys_iter()
// Usage:
//   result = _ps_refine_edge_keys_iter(poly, keys, face_keys, vert_keys, edges, edge_faces, max_iter);
// Description:
//   Iterative edge-key refinement until stable keys or budget exhausted.
//   .
//   - Returns: refined edge keys.
// Arguments:
//   poly = descriptor.
//   keys = initial edge keys.
//   `face_keys` =
//   `vert_keys`: current neighbor keys. =
//   `edges` =
//   `edge_faces`: adjacency. =
//   max_iter = iteration budget.
...
// Function: _ps_refine_vert_keys_iter()
// Usage:
//   result = _ps_refine_vert_keys_iter(poly, keys, face_keys, edges, edge_faces, max_iter);
// Description:
//   Iterative vertex-key refinement until stable families or budget exhausted.
//   .
//   - Returns: refined vertex keys.
// Arguments:
//   poly = descriptor.
...
// Function: poly_classify()
// Usage:
//   result = poly_classify(poly, detail, eps, radius, include_geom);
// Description:
//   Classify polyhedral faces/edges/vertices into structural families.
//   .
//   - Returns: classification tuple `[face_families, edge_families, vert_families]`
//     where each families list is `[[key, idxs], ...]`.
//   .
//   - Approach: build topology base keys for each element type,
...
// Function: ps_classify_face_families()
// Usage:
//   result = ps_classify_face_families(cls);
// Description:
//   Safe accessor for face families from classification tuple.
//   .
//   - Returns: face families list or `[]`.
// Arguments:
...
// Function: ps_classify_face_ids()
// Usage:
//   result = ps_classify_face_ids(cls, n_faces);
// Description:
//   Build per-face family-id map from classification.
//   .
//   - Returns: id map length `n_faces`.
// Arguments:
//   cls = classification tuple.
...
// Function: ps_classify_edge_ids()
// Usage:
//   result = ps_classify_edge_ids(cls, n_edges);
// Description:
//   Build per-edge family-id map from classification.
//   .
//   - Returns: id map length `n_edges`.
// Arguments:
//   cls = classification tuple.
...
// Function: ps_classify_vert_ids()
// Usage:
//   result = ps_classify_vert_ids(cls, n_verts);
// Description:
//   Build per-vertex family-id map from classification.
//   .
//   - Returns: id map length `n_verts`.
// Arguments:
//   cls = classification tuple.
...
// Function: ps_classify_face_idxs_by_n()
// Usage:
//   result = ps_classify_face_idxs_by_n(cls, n);
// Description:
//   Convenience selector: collect face indices by face arity.
//   .
//   - Returns: concatenated face index list across all matching face families.
// Arguments:
//   cls = classification tuple.
...
// Function: _ps_classification_family_describe_str()
// Usage:
//   result = _ps_classification_family_describe_str(label, idx, fam, detail, kvpair_to_str, field_sep);
// Description:
